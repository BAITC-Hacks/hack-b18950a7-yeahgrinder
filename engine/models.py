...
class Params(Model):
    ...
    lead_time_days: dict[Supplier, int] = Field(default_factory=lambda: {"IEK": 30, "Systeme Electric": 45})
    service_z: float = Field(default=1.65, ge=0, le=4)
    ...
    def valid_ranges(self):
        ...
        if any(isinstance(v, bool) or not 0 <= v <= 365 for v in self.lead_time_days.values()):
