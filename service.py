...
from engine.run import SUPPLIER_IN, Result, compute, fingerprint
...
class ProcurementService:
    ...
    def data(self, params):
        if self.dataset is not None:
            return self.dataset
        return load(self.paths) if self.paths else load()
    ...
    def calculate(self, params=None, user_id='local'):
        ...
        self.memory.add_run(run_id, params_json, config_hash, fingerprint(ds), result.warnings)
    ...
    def snapshot(self, run_id):
        ...
        with open(self.runs / metadata['run_id'] / 'dataset.pkl', 'rb') as f:
            return pickle.load(f)  # свежая копия: what-if не меняет исходный снимок
    ...
    def what_if(self, run_id, supplier=None, sku=None, lead_time_days=None, review_days=None, service_z=None, extra_transit=None):
        ...
        targets = ds.items
        if supplier:
            targets = targets[targets.supplier == SUPPLIER_IN.get(supplier, supplier)]
        if sku:
        ...
        if extra_transit is not None:
            ...
            item = targets.iloc[0]
            extra = pd.DataFrame([{'code': item.code, 'doc': 'what-if', 'qty': float(extra_transit),
                                    'order_date': pd.NaT, 'arrival_date': ds.as_of + pd.Timedelta(days=1)}])
            ds.transit = pd.concat([ds.transit, extra], ignore_index=True) if len(ds.transit) else extra
