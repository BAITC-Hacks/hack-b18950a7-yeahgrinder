...
const state={page:'Сегодня',review:false,restored:false,overstock:false,tab:'recommendations',query:'',supplier:'Все',category:'Все',risk:'Все',transit:'Все',oneoff:'Все',abc:'Все',selected:new Set(),productId:null,detailTab:'recommendation',chartId:'iek-1',showSpike:false};
...
function navigate(page,tab='recommendations'){state.page=page;state.tab=tab;state.query='';state.supplier='Все';state.category='Все';state.risk='Все';state.transit='Все';state.oneoff='Все';state.abc='Все';state.review=false;state.restored=false;state.overstock=false;state.selected.clear();render();window.scrollTo({top:0,behavior:'instant'});}
...
function todayItem(p){return `<button class="today-item" data-action="product" data-id="${p.id}"><span class="ti-name">${esc(p.name)}<small>${esc(p.article)}</small></span><span class="ti-stock">${(()=>{const r=runout(p);return r?(p.stock_current?`до ${dmy(r.out)}`:'нет в наличии')+(r.gap>0?` · <b class="cover-none">−${r.gap} дн</b>`:''):'—';})()}</span><span class="ti-qty">${unit(quantity(p),p)}</span></button>`;}

// --- даты «закончится / придёт» и деньги (только где есть себестоимость)
function addDays(n){const d=demo.asOf?new Date(demo.asOf+'T12:00:00'):new Date();d.setDate(d.getDate()+Math.round(n));return d;}
const dmy=d=>d.toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit'});
function runout(p){
 if(p.cover_days==null)return null;
 const out=addDays(p.cover_days), arrive=addDays(p.lead_time_days||0), gap=Math.max(0,Math.round((p.lead_time_days||0)-p.cover_days));
 return {out,arrive,gap};
}
function runoutText(p){const r=runout(p);if(!r)return '';return r.gap>0?`закончится ${dmy(r.out)}, поставка при заказе сегодня — ${dmy(r.arrive)} · <b class="cover-none">${r.gap} дн. без товара</b>`:`закончится ${dmy(r.out)}, поставка успевает (${dmy(r.arrive)})`;}
function isOverstock(p){return (p.base_monthly||0)>0&&p.stock_current>6*p.base_monthly;}
function moneyCard(ps){
 const priced=ps.filter(p=>p.unit_cost);if(!priced.length)return '';
 const sup=[...new Set(priced.map(p=>p.supplier))].map(supplierName).join(', ');
 const over=priced.filter(isOverstock), overSum=over.reduce((a,p)=>a+(p.stock_current-6*p.base_monthly)*p.unit_cost,0);
 const risk=priced.filter(p=>p.urgency==='CRITICAL'), riskSum=risk.reduce((a,p)=>a+Math.max(0,p.forecast_horizon-p.stock_current-p.in_transit_in_horizon)*p.unit_cost,0);
 return `<article class="today-card money"><div class="today-card-head"><span class="today-step">₸</span><div><div class="today-tag">💰 Деньги · ${esc(sup)}</div><h2>Что стоит на кону</h2><p>Считаем по себестоимости из файла менеджера; у ИЭК цен в данных нет.</p></div></div><div class="money-grid"><button class="kpi alert" data-action="today-open" data-supplier="${esc(priced[0].supplier)}" title="Продажи по критичным позициям, которые не покрыты остатком и путём до горизонта"><span>Под риском до поставки</span><strong>${money(riskSum)}</strong><small>${positions(risk.length)} критично</small></button><button class="kpi calm" data-action="overstock" data-supplier="${esc(priced[0].supplier)}" title="Остаток сверх 6 месяцев спроса × себестоимость"><span>Заморожено в излишках</span><strong>${money(overSum)}</strong><small>${positions(over.length)} с запасом > 6 мес.</small></button></div><div class="context-note">${icon('info')} Излишки — деньги, которые можно не тратить на следующий заказ; риск — продажи, которые уйдут к конкуренту без заказа сегодня.</div></article>`;
}
...
// --- козырь: проверка расчёта на истории (данные партнёра, meta.backtest)
function backtestCard(compact=false){
 const b=demo.backtest;if(!b||!b.real)return '';
 const parts=[['критично',b.flagged['критично'],'crit'],['высокая',b.flagged['высокая'],'high'],['плановая',b.flagged['плановая'],'plan'],['пропущено',b.missed,'miss']].filter(x=>x[1]>0);
 const bar=parts.map(([l,v,c])=>`<i class="bt-seg ${c}" style="flex:${v}" title="${l}: ${v}"></i>`).join('');
 return `<section class="card backtest ${compact?'compact':''}"><div class="bt-head"><div><div class="today-tag">🧪 Проверено на истории</div><h2>Система предупредила бы о <strong>${b.warned_pct}%</strong> реальных дефицитов за месяц</h2><p>Расчёт откатили на <strong>${esc(b.as_of)}</strong> и сравнили с тем, что случилось: в ${esc(b.window)} на складе закончились <strong>${fmt(b.real)}</strong> товаров. Из них <strong>${fmt(b.warned)}</strong> уже стояли бы в списке к заказу (${b.flagged['критично']} «критично», ${b.flagged['высокая']} «высокая», ${b.flagged['плановая']} «плановая»), ${fmt(b.missed)} система пропустила бы.</p></div><div class="bt-num"><strong>${b.warned_pct}%</strong><span>${fmt(b.warned)} из ${fmt(b.real)}</span></div></div><div class="bt-bar">${bar}</div><div class="bt-legend"><span><i class="crit"></i>критично</span><span><i class="high"></i>высокая</span><span><i class="plan"></i>плановая</span><span><i class="miss"></i>пропущено</span></div>${!compact&&b.examples?.length?`<div class="bt-examples"><div class="card-sub">Примеры: помечены «критично» ${esc(b.as_of)} — и действительно закончились</div>${b.examples.map(e=>`<button class="today-item" data-action="product" data-id="${esc(e.sku)}"><span class="ti-name">${esc(e.name)}<small>${esc(e.supplier)}</small></span><span class="ti-qty">${fmt(e.order_qty)} ${esc(e.unit)}</span></button>`).join('')}</div>`:''}<div class="context-note">${icon('info')} Ложные тревоги не считаем: в те месяцы менеджер заказывал сам, и часть «критичных» он просто спас. Это проверка на данных партнёра, не демо-цифра.</div></section>`;
}
// --- админка: параметры расчёта, правила менеджера, история
const admin={state:null,loading:false,error:null,saving:false};
async function loadAdmin(){admin.loading=true;try{const r=await fetch('./admin/state');const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(apiError(d,r));admin.state=d;admin.error=null;}catch(e){admin.error=e.message;}admin.loading=false;if(state.page==='Админ')render();}
function adminPage(){
 if(!demo.live)return header('Админ','Управление параметрами расчёта')+`<div class="notice">${aiOff().replace('Помощник выключен: не задан ключ OpenAI.','Админка работает только с сервером расчёта.')}</div>`;
 if(!admin.state&&!admin.loading&&!admin.error)loadAdmin();
 if(admin.error)return header('Админ','Управление параметрами расчёта')+`<div class="notice"><div>${icon('alert')}</div><div><strong>Не удалось загрузить</strong>${esc(admin.error)}</div></div>`;
 if(!admin.state)return header('Админ','Управление параметрами расчёта')+'<div class="ai-loading"><i></i><i></i><i></i> Загружаем настройки…</div>';
 const st=admin.state, pref=st.preferences||{}, def=st.defaults, val=k=>pref[k]??def[k];
 const lead=s=>(pref.lead_time_days??def.lead_time_days??{})[s];
 const params=`<article class="card admin-card"><h2>Параметры расчёта</h2><p class="card-sub">Применяются ко всем расчётам: интерфейсу, API и помощнику. Пустое поле — значение из данных.</p><div class="admin-grid"><label>Период до следующего заказа, дн.<input id="adm-review" type="number" min="1" max="365" value="${val('review_days')??30}"></label><label>Срок поставки ИЭК, дн.<input id="adm-lead-iek" type="number" min="0" max="365" placeholder="из заказов в пути" value="${lead('IEK')??''}"></label><label>Срок поставки Systeme Electric, дн.<input id="adm-lead-se" type="number" min="0" max="365" placeholder="допущение 45" value="${lead('Systeme Electric')??''}"></label><label>План прироста спроса, %<input id="adm-growth" type="number" min="-50" max="100" step="1" value="${val('growth_pct')??0}"></label><label>Уровень сервиса, z (пусто — по категориям)<input id="adm-z" type="number" min="0" max="4" step="0.05" placeholder="1/A 98%, 2/B 95%, 3/C 90%" value="${pref.service_z??''}"></label></div><div class="today-actions">${btn(admin.saving?'Сохраняем…':'Сохранить и пересчитать','admin-save','primary',admin.saving?'disabled':'')}${Object.keys(pref).length?btn('Сбросить к настройкам по умолчанию','admin-reset','small'):''}</div></article>`;
 const rules=`<article class="card admin-card"><h2>Правила менеджера</h2><p class="card-sub">«Минимум N по товару»: расчёт не опустит заказ ниже, пока есть спрос. Правило видно в обосновании.</p>${st.rules.length?`<table class="bd-table admin-table"><thead><tr><th>Поставщик</th><th>Товар</th><th class="num">Минимум</th><th></th></tr></thead><tbody>${st.rules.map(r=>{const p=allProducts.find(x=>x.sku===r.sku);return `<tr><td>${esc(supplierName(r.supplier))}</td><td>${p?`<button class="check-link" data-action="product" data-id="${esc(p.id)}">${esc(p.name)}</button>`:esc(r.sku)}<br><small class="muted">${esc(r.sku)}</small></td><td class="num"><strong>${fmt(r.min_qty)}</strong>${p?' '+esc(p.unit):''}</td><td class="num">${btn('Удалить','admin-rule-delete','text small',`data-supplier="${esc(r.supplier)}" data-sku="${esc(r.sku)}"`)}</td></tr>`;}).join('')}</tbody></table>`:'<p class="muted">Правил пока нет.</p>'}<div class="admin-grid" style="margin-top:12px"><label>Код 1С товара<input id="adm-rule-sku" list="adm-skus" placeholder="например, ярп40257"><datalist id="adm-skus">${allProducts.filter(p=>p.recommended_qty>0).slice(0,400).map(p=>`<option value="${esc(p.sku)}">${esc(p.name)}</option>`).join('')}</datalist></label><label>Минимум к заказу<input id="adm-rule-qty" type="number" min="0" step="1" placeholder="кратно упаковке"></label></div><div class="today-actions">${btn(icon('plus')+' Добавить правило','admin-rule-add','small')}</div></article>`;
 const fmtDate=t=>new Date(t).toLocaleString('ru-RU',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'});
 const history=`<article class="card admin-card"><h2>История</h2><p class="card-sub">Расчёты и решения, сохранённые сервером.</p><div class="admin-cols"><div><div class="today-tag">Решения (${st.approvals.length})</div>${st.approvals.length?`<ul class="admin-list">${st.approvals.map(a=>`<li><strong>${a.decision==='approve'?'✓ Утверждено':a.decision==='reject'?'✕ Отклонено':'✎ Правка'}</strong> · ${esc(supplierName(a.supplier))} · ${esc(a.user_id)}<small>${fmtDate(a.created_at)}${a.reason?` · ${esc(a.reason)}`:''}</small></li>`).join('')}</ul>`:'<p class="muted">Серверных решений нет. Утверждения из интерфейса хранятся в браузере.</p>'}</div><div><div class="today-tag">Расчёты (${st.runs.length})</div>${st.runs.length?`<ul class="admin-list">${st.runs.map(r=>`<li><strong>${esc(r.run_id.slice(0,8))}</strong><small>${fmtDate(r.created_at)} · данные ${esc(r.data_hash.slice(0,8))}</small></li>`).join('')}</ul>`:'<p class="muted">Расчётов пока нет.</p>'}</div></div></article>`;
 return header('Админ','Параметры, правила и история — без доступа к коду')+`<div class="admin-layout">${params}${rules}${history}</div>`;
}
async function adminSave(reset=false){
 admin.saving=true;render();
 const num=id=>{const v=$(id)?.value.trim();return v===''?null:Number(v);};
 const body=reset?{}:{review_days:num('#adm-review'),growth_pct:num('#adm-growth'),service_z:num('#adm-z'),lead_time_days:{IEK:num('#adm-lead-iek'),'Systeme Electric':num('#adm-lead-se')}};
 try{await apiPost('./admin/preferences',body);toast(reset?'Настройки сброшены. Пересчитываем…':'Сохранено. Пересчитываем…');runPromise=null;ai.brief=null;location.reload();}
 catch(e){admin.saving=false;toast(e.message);render();}
}
function overview(){
...
function render(){
 ...
 $('#page').innerHTML=state.page==='Сегодня'?today():state.page==='Обзор'?overview():state.page==='Закупки'?purchases():state.page==='Товары'?catalog():state.page==='Админ'?adminPage():dataPage();
...
const actions={
 ...
 'clear-filters':()=>{state.query='';state.supplier='Все';state.category='Все';state.risk='Все';state.transit='Все';state.oneoff='Все';state.abc='Все';state.review=false;state.restored=false;state.overstock=false;state.selected.clear();render();},
 'clear-overstock':()=>{state.overstock=false;render();},
 'overstock':b=>{navigate('Товары');Object.assign(state,{overstock:true,supplier:b.dataset.supplier||'Все'});render();},
 ...
 'admin-save':()=>adminSave(false),
 'admin-reset':()=>adminSave(true),
 'admin-rule-add':async()=>{const sku=$('#adm-rule-sku').value.trim(),qty=Number($('#adm-rule-qty').value);const p=allProducts.find(x=>x.sku===sku);if(!p){toast('Товар с таким кодом не найден');return;}if(!Number.isInteger(qty)||qty<0){toast('Минимум — целое число ≥ 0');return;}try{await apiPost('./admin/rules',{supplier:p.supplier,sku,min_qty:qty});toast('Правило сохранено. Пересчитываем…');location.reload();}catch(e){toast(e.message);}},
 'admin-rule-delete':async b=>{try{const r=await fetch(`./admin/rules?supplier=${encodeURIComponent(b.dataset.supplier)}&sku=${encodeURIComponent(b.dataset.sku)}`,{method:'DELETE'});if(!r.ok)throw new Error('Сервис ответил '+r.status);toast('Правило удалено. Пересчитываем…');location.reload();}catch(e){toast(e.message);}},
