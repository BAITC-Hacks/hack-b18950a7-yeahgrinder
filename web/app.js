...
const actions={
 ...
 'ask-fill':b=>{chats[state.productId]={...(chats[state.productId]||{}),q:b.dataset.q};renderProduct();$('#ask-input')?.focus();},
 'ask-send':async()=>{const p=find(state.productId);const q=$('#ask-input').value.trim();if(!q)return;chats[p.id]={q,loading:true};renderProduct();
