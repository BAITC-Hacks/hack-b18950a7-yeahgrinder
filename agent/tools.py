...
def make_tools(service, run_id: str):
    ...
    tables = {"orders": orders.drop(columns=["excluded_events", "restored_events", "forecast_months"])
              ...
              "sales_monthly": snapshot.sales_monthly, "stock_monthly": snapshot.stock_monthly,
              "transit": snapshot.transit, "sku_master": snapshot.sku_master.assign(flags=snapshot.sku_master["flags"].map(", ".join))}
