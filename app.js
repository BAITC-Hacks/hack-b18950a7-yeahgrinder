...
function supplierKpis(ps){
 const cards=suppliers.map((s,i)=>{const rows=ps.filter(p=>p.supplier===s);if(!rows.length)return '';
  const crit=rows.filter(p=>p.urgency==='CRITICAL'),value=rows.reduce((a,p)=>a+(p.order_value||0),0);
  const tiles=[['К заказу',fmt(rows.filter(p=>p.recommended_qty>0).length),'order','recommendations'],['Критично',fmt(crit.length),'critical','urgent'],
   ['Критично и ничего нет в пути',fmt(crit.filter(p=>!p.in_transit_in_horizon).length),'bare','urgent-bare'],
   ['Разовые сделки исключены',fmt(rows.filter(p=>p.excluded_events?.length).length),'oneoff','oneoff'],...(value?[['Сумма заказа',money(value),'value','recommendations']]:[])];
  const estimated=rows.some(p=>(p.warnings||[]).some(w=>w.startsWith('Остаток оценочный')));
  return `<article class="card supplier-kpi"><div class="card-heading"><div style="display:flex;gap:12px;align-items:center"><div class="supplier-logo ${i?'se':''}">${i?'SE':'IEK'}</div><div><h2>${s}</h2><div class="card-sub">${fmt(rows.length)} товаров · срок поставки ${esc(demo.leadText?.[s]||(i?'45 дней · допущение':'24–30 дней · демо-срок'))}</div></div></div></div><div class="kpi-grid">${tiles.map(([label,value,tip,action])=>`<button class="kpi" data-action="${action}" data-supplier="${s}" title="${esc(KPI_TIPS[tip])}"><span>${label}</span><strong>${value}</strong><small>${KPI_TIPS[tip]}</small></button>`).join('')}</div>${estimated?`<div class="context-note">${icon('info')} Остаток ${i?'части товаров':'ИЭК'} — оценка: приходы за сентябрь в выгрузке не видны. Расчёт выполнен по доступному срезу данных — остатки и приходы могут изменить рекомендацию.</div>`:''}</article>`;}).join('');
