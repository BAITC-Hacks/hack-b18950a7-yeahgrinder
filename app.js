...
const state={page:'Обзор',tab:'recommendations',query:'',supplier:'Все',category:'Все',risk:'Все',selected:new Set(),productId:null,detailTab:'recommendation',chartId:'iek-1',showSpike:false};
...
function filtered(){const q=state.query.trim().toLocaleLowerCase('ru');return products().filter(p=>(state.supplier==='Все'||p.supplier===state.supplier)&&(state.category==='Все'||p.category===state.category)&&(!q||`${p.name} ${p.article} ${p.sku} ${p.supplier}`.toLocaleLowerCase('ru').includes(q)));}
...
function table(rows,{select=false,catalog=false}={}){
 ...
 return `<div class="table-wrap"><table><thead><tr>${select?`<th class="check-cell"><input type="checkbox" id="select-all" aria-label="Выбрать все показанные рекомендации" ${selectable.length&&selectable.every(p=>state.selected.has(p.id))?'checked':''}></th>`:''}<th>Товар</th><th class="num">Остаток</th><th class="num">В пути</th><th class="num">Запаса хватит</th><th class="num">${catalog?'Рекомендация':'К заказу'}</th><th>Срочность</th><th></th></tr></thead><tbody>${rows.map(p=>`<tr data-row="${p.id}" class="${state.selected.has(p.id)?'selected':''}">${select?`<td class="check-cell"><input type="checkbox" data-select="${p.id}" aria-label="Выбрать ${esc(p.name)}" ${state.selected.has(p.id)?'checked':''} ${saved.rejected.includes(p.id)?'disabled':''}></td>`:''}<td class="product"><button class="product-open" data-action="product" data-id="${p.id}"><strong>${esc(p.name)}</strong><small>${esc(p.article)} · ${p.supplier}</small>${p.quality==='Нужна проверка'?'<span class="quality-dot">Нужна проверка данных</span>':''}</button></td><td class="num">${unit(p.stock_current,p)}</td><td class="num">${p.in_transit_in_horizon?unit(p.in_transit_in_horizon,p):'—'}</td><td class="num">${days(p.cover_days)}</td><td class="num order">${saved.rejected.includes(p.id)?'<span class="muted">Отклонено</span>':quantity(p)?unit(quantity(p),p):'—'}${changed(p)?'<small class="edited" style="display:block;margin-top:4px;font-size:10px">Изменено вами</small>':''}${saved.draft.includes(p.id)?'<small style="display:block;margin-top:4px;font-size:10px">В черновике</small>':''}</td><td>${badge(p)}</td><td><button class="btn text small" data-action="product" data-id="${p.id}" aria-label="Открыть ${esc(p.name)}">${icon('chevron')}</button></td></tr>`).join('')}</tbody></table><div class="table-info">${positions(rows.length)} · Количества указаны в единицах товара · Нажмите на строку для объяснения</div></div>`;
}
...
function overview(){
 ...
 const cards=[['Товаров в анализе',ps.length,'2 поставщика · демо-набор','box','','catalog'],['Высокая срочность',critical+counts.HIGH,`Критичных позиций: ${critical}`,'alert','warn','high-priority'],['Рекомендовано к заказу',needs.length,'позиций для проверки','bag','blue','recommendations'],['Товары в пути',transit,'позиций с поступлениями','truck','good','transit']];
 ...
 <section class="suppliers-section"><div class="supplier-heading"><h2>Поставщики</h2><span>Сводка по ${ps.length} демо-позициям</span></div><div class="supplier-cards">${suppliers.map((s,i)=>{const rows=ps.filter(p=>p.supplier===s);return `<article class="supplier-card"><div class="supplier-logo ${i?'se':''}">${i?'SE':'IEK'}</div><div class="supplier-info"><strong>${s}</strong><small>${i?'45 дней · допущение':'24–30 дней · демо-срок'}</small></div><div class="supplier-stats"><div class="supplier-stat"><strong>${rows.length}</strong><small>товаров</small></div><div class="supplier-stat"><strong>${rows.filter(p=>p.recommended_qty>0).length}</strong><small>к заказу</small></div><div class="supplier-stat"><strong>${rows.filter(p=>p.urgency==='CRITICAL').length}</strong><small>критичных</small></div></div></article>`;}).join('')}</div></section>`;
}
function recommendations(){
 ...
 rows.sort((a,b)=>urgency[a.urgency].rank-urgency[b.urgency].rank);
...
function detailsBody(p){
 if(state.detailTab==='history')return `<div class="card-sub">${p.id==='iek-loop'?'Архивный демо-кейс · апрель — ноябрь 2025':'Демо · апрель — ноябрь 2026; сентябрь — неполный месяц'} · ${p.unit}<br>График базового сценария</div>${chart(p,{detail:true,spike:state.showSpike})}${p.excluded_events.map(e=>`<div class="event-card"><strong>Разовая крупная продажа скорректирована</strong><br>${e.date} · накладная ${e.doc}<br>Продажа: <strong>${unit(e.qty,p)}</strong> → учтено ${unit(e.threshold,p)}<br>Из регулярного спроса исключено ${unit(e.excess,p)}</div>`).join('')}${p.lost_qty?`<div class="notice"><div>${icon('info')}</div><div><strong>Восстановлен упущенный спрос</strong>Июль: факт 4 шт. → скорректированный спрос 52 шт.<br>Восстановлено ${unit(p.lost_qty,p)} из-за отсутствия товара.</div></div>`:''}<details class="disclosure"><summary>Посмотреть значения по месяцам</summary><table class="history-table"><thead><tr><th>Месяц</th><th class="num">Факт</th><th class="num">Скорр.</th><th class="num">Прогноз</th></tr></thead><tbody>${p.history.map(s=>`<tr><td>${s.month}</td><td class="num">${s.raw===null?'—':fmt(s.raw)}</td><td class="num">${s.clean===null?'—':fmt(s.clean)}</td><td class="num">${s.forecast===null?'—':fmt(s.forecast)}</td></tr>`).join('')}</tbody></table></details>`;
 if(state.detailTab==='data')return `<div class="notice"><div>${icon('alert')}</div><div><strong>${p.quality}</strong>${p.warnings.map(esc).join('<br>')}</div></div><div class="details-grid"><div class="detail-tile">Код 1С<strong>${p.sku}</strong></div><div class="detail-tile">Единица измерения<strong>${p.unit}</strong></div><div class="detail-tile">Срок поставки<strong>${p.lead_time_days} дней</strong></div><div class="detail-tile">Кратность заказа<strong>${unit(p.moq,p)}</strong></div></div><p class="reason">Все значения этой карточки заданы в демо-наборе. Они не являются результатом обработки реальных файлов.</p><details class="disclosure" open><summary>Источники и актуальность</summary><p>Продажи и транзит: демо-снимок на 22.09.2026.<br>Остатки: демонстрационная оценка.<br>${p.id==='iek-loop'?'График LOOP показывает отдельный архивный кейс 2025 года.':''}</p></details>`;
 const rounded=Math.max(0,p.need);
 return `<div class="recommendation-hero"><div class="label">Рекомендуем ${p.recommended_qty?'заказать':'сохранить текущий запас'}</div><div class="value">${fmt(p.recommended_qty)} <span>${p.unit}</span></div><p>${p.recommended_qty?`На горизонт ${demo.horizon} дней · кратность ${unit(p.moq,p)}`:'Дополнительный заказ не требуется'}${saved.scenario==='growth'?' · сценарий +20%':''}</p></div><div class="section-label">Почему именно столько</div><div class="breakdown">${[['Прогноз на горизонт',p.forecast_horizon,''],['Страховой запас',p.safety_stock,'+'],['Текущий остаток',p.stock_current,'−'],['Товар в пути',p.in_transit_in_horizon,'−']].map(([label,n,sign])=>`<div class="breakdown-row"><span>${label}</span><strong>${sign} ${unit(n,p)}</strong></div>`).join('')}<div class="breakdown-row"><span>Потребность до округления</span><strong>${unit(rounded,p)}</strong></div><div class="breakdown-row total"><span>${p.need<=0?'Запаса достаточно':'С учётом кратности'}</span><strong>${unit(p.recommended_qty,p)}</strong></div></div><p class="reason">${esc(p.reason_text)}</p><div class="details-grid"><div class="detail-tile">Запаса хватит<strong>${days(p.cover_days)}</strong></div><div class="detail-tile">Срок поставки<strong>${days(p.lead_time_days)}</strong></div></div>${saved.scenario==='growth'?'<div class="notice info">Срочность, покрытие и графики показаны для базового сценария. Количество +20% задано заранее.</div>':''}<details class="disclosure"><summary>${p.quality} · посмотреть оговорки</summary><p>${p.warnings.map(esc).join('<br>')}</p></details><div class="edit-form"><label for="detail-qty">Ваше количество к заказу</label><div class="qty"><input id="detail-qty" type="number" min="0" step="1" value="${quantity(p)}">${p.unit}</div><div class="field-hint">Рекомендация ${unit(p.recommended_qty,p)} останется видимой. Кратность: ${unit(p.moq,p)}</div><label for="detail-comment" style="margin-top:14px">Комментарий к решению</label><textarea id="detail-comment" maxlength="500" placeholder="Например: согласовано с менеджером проекта">${esc(comment(p))}</textarea><div id="detail-error" class="field-error" role="alert"></div>${btn('Сохранить правку','save-edit','small')}</div>`;
}
function renderProduct(){
 const p=find(state.productId);if(!p)return;
 const d=$('#product-dialog');
 d.innerHTML=`<header class="drawer-header"><div><div class="eyebrow">${p.supplier} / ${p.category}</div><h2>${esc(p.name)}</h2><p>${esc(p.article)} · код 1С ${p.sku}</p><div style="margin-top:12px">${badge(p)} ${saved.rejected.includes(p.id)?'<span class="mode-pill">Отклонено вами</span>':''}</div></div><button class="icon-btn" data-action="close-product" aria-label="Закрыть карточку">${icon('close')}</button></header><div class="drawer-body"><div class="tabs">${[['recommendation','Рекомендация'],['history','История спроса'],['data','Данные и допущения']].map(([k,v])=>`<button class="tab ${state.detailTab===k?'active':''}" data-action="detail-tab" data-tab="${k}">${v}</button>`).join('')}</div>${detailsBody(p)}</div><footer class="drawer-footer">${saved.rejected.includes(p.id)?btn('Вернуть рекомендацию','restore','small'):btn('Отклонить','reject','text small')}${btn('Изменить','edit-focus','small')}${btn(saved.draft.includes(p.id)?icon('check')+' В черновике':icon('plus')+' В черновик','add-product','primary')}</footer>`;
}
function openProduct(id,tab='recommendation'){state.productId=id;state.detailTab=tab;state.showSpike=false;renderProduct();$('#product-dialog').showModal();}
...
const actions={
 ...
 'clear-filters':()=>{state.query='';state.supplier='Все';state.category='Все';state.risk='Все';state.selected.clear();render();},
 ...
 'approve':()=>{if(!$('#acknowledge')?.checked)return;const rows=saved.draft.map(find).filter(p=>p&&quantity(p)>0);saved.approved={at:new Date().toISOString(),rows:rows.map(p=>[p.sku,p.article,p.name,p.unit,quantity(p),p.supplier,urgency[p.urgency].label])};persist();$('#confirm-dialog').close();render();toast('Заказ утверждён локально. XLSX готов к скачиванию.');},
 'export':exportApproved,'settings':settings,'close-settings':()=>$('#settings-dialog').close(),
...
document.addEventListener('input',e=>{if(e.target.id==='list-search'){state.query=e.target.value;state.selected.clear();render();}});
document.addEventListener('change',e=>{
 ...
 if(t.id==='acknowledge')$('#approve-button').disabled=!t.checked;
});
...
document.addEventListener('mouseover',showChartPoint);document.addEventListener('focusin',showChartPoint);
...
async function start(){
 $('#page').innerHTML=empty('Загружаем демо-набор','Подготавливаем рекомендации и историю спроса.');
 try{
